...
export default function MapScreen({ user }) {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (map.current) {
          ...
          // Agregar pin de ubicación del usuario
          if (userMarker.current) userMarker.current.remove();
          const el = document.createElement("div");
          el.style.cssText = `
            width: 16px; height: 16px; border-radius: 50%;
            background: #7b7fd4; border: 3px solid white;
            box-shadow: 0 0 0 3px rgba(123,127,212,0.3);
          `;
          userMarker.current = new window.mapboxgl.Marker(el)
            .setLngLat(coords)
            .addTo(map.current);
        }
      },
  ...
  // Cambiar zoom cuando cambia el radio
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      map.current.on("load", () => {
        ...
        if (userLocation) {
          map.current.flyTo({ center: userLocation, zoom: ZOOM_POR_RADIO[radio] });
        }
      });
    };
  ...
  return (
    <div style={{ height: "100%", position: "relative", background: "#f0f0f8" }}>

      {/* Header */}
      <div style={{
        position: "absolute", top: 0, left: 0, right: 0, zIndex: 10,
        padding: "48px 20px 12px",
        background: "linear-gradient(to bottom, rgba(240,240,248,0.98) 75%, transparent)",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <img src="/logo-vector.svg" alt="RESCAT" style={{ height: 36 }} />
          <div style={{ textAlign: "right" }}>
            <p style={{ margin: 0, fontSize: 12, color: "#7b7fd4" }}>HOLA, {nombre}</p>
            <div style={{ display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end" }}>
              <span style={{ fontSize: 13 }}>🌐</span>
              <span style={{ fontSize: 16, fontWeight: 700, color: "#1e2a4a" }}>240 DC</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[2, 5, 10].map(km => (
            <button key={km} onClick={() => setRadio(km)} style={{
              ...
              background: radio === km ? "#1e2a4a" : "rgba(255,255,255,0.85)",
      ...
      {selectedInsumo && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
            ...
            <button onClick={() => setSelectedInsumo(null)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "#aaa", fontSize: 22, lineHeight: 1 }}>×</button>
          </div>
          ...
          <div style={{ background: "#f0f0f8", borderRadius: 12, padding: "16px", marginBottom: 14, height: 80, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 13, color: "#7b7fd4" }}>📦 Foto del insumo</span>
          </div>
