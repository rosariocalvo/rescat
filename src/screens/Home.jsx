...
function TabHistorial({ user, onBack }) {
  ...
  useEffect(() => {
    supabase.from("publicaciones").select("*").eq("user_id", user.id)
      .order("created_at", { ascending: false })
      .then(({ data }) => { setHistorial(data || []); setLoading(false); });
  }, []);

  const formatFecha = (iso) => new Date(iso).toLocaleDateString("es-CL", { day: "numeric", month: "short", year: "numeric" });
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
      ...
      <div style={{ padding: "52px 24px 20px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#1e2a4a", fontFamily: "Outfit, sans-serif" }}>Mi actividad</h2>
      </div>
      <div style={{ padding: "0 24px" }}>
        {loading && <p style={{ color: "#b0b8d0", fontFamily: "Outfit, sans-serif" }}>Cargando...</p>}
        {!loading && historial.length === 0 && (
          <p style={{ color: "#b0b8d0", fontFamily: "Outfit, sans-serif", fontSize: 14 }}>Aún no tienes actividad registrada.</p>
        )}
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {historial.map((p) => (
            <div key={p.id} style={{ background: "white", borderRadius: 16, padding: "16px", boxShadow: "0 1px 8px rgba(30,42,74,0.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: p.tipo === "compartir" ? "#f0f1f9" : "#fff0f2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {p.tipo === "compartir" ? (
                      ...
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#EC6765" strokeWidth="1.5"/><line x1="12" y1="9" x2="12" y2="13" stroke="#EC6765" strokeWidth="1.5" strokeLinecap="round"/><line x1="12" y1="17" x2="12.01" y2="17" stroke="#EC6765" strokeWidth="2" strokeLinecap="round"/></svg>
                    )}
                  </div>
                  <div>
                    <span style={{ fontSize: 11, fontWeight: 600, fontFamily: "Outfit, sans-serif", color: p.tipo === "compartir" ? "#7890D0" : "#EC6765" }}>
                      {p.tipo === "compartir" ? "Compartiste" : "Solicitaste"}
                    </span>
                    <p style={{ margin: "2px 0", fontWeight: 600, fontSize: 14, color: "#1e2a4a", fontFamily: "Outfit, sans-serif" }}>{p.nombre_insumo}</p>
                    {p.cantidad && <p style={{ margin: 0, fontSize: 12, color: "#7b80a0", fontFamily: "Outfit, sans-serif" }}>Cantidad: {p.cantidad}</p>}
                  </div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <span style={{ fontSize: 11, color: "#b0b8d0", fontFamily: "Outfit, sans-serif", display: "block", marginBottom: 4 }}>{formatFecha(p.created_at)}</span>
                  <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: p.estado === "activa" ? "#e8f5e9" : "#f5f5f5", color: p.estado === "activa" ? "#4caf50" : "#9e9e9e", fontFamily: "Outfit, sans-serif" }}>
                    {p.estado === "activa" ? "Activa" : "Cerrada"}
                  </span>
                </div>
              </div>
              {p.mensaje && <p style={{ margin: "10px 0 0", fontSize: 13, color: "#7b80a0", fontFamily: "Outfit, sans-serif", lineHeight: 1.4 }}>"{p.mensaje}"</p>}
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
      ...
      <div style={{ padding: "52px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ background: "white", borderRadius: 50, padding: "8px 18px", display: "flex", alignItems: "center", gap: 8, boxShadow: "0 1px 8px rgba(30,42,74,0.10)" }}>
          <IconoDC />
      ...
      <div style={{ padding: "20px 24px 0" }}>
        <p style={{ fontSize: 22, fontWeight: 700, color: "#1e2a4a", margin: "0 0 4px", fontFamily: "Outfit, sans-serif" }}>HOLA, {firstName} 👋</p>
        ...
        <div onClick={() => window.dispatchEvent(new CustomEvent("openMapa"))}
          style={{ background: "#e8eaf0", borderRadius: 20, padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "stretch", marginBottom: 32, cursor: "pointer", gap: 12 }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{ marginBottom: 8 }}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="#1e2a4a"/></svg>
              <p style={{ margin: "0 0 4px", fontSize: 17, fontWeight: 700, color: "#1e2a4a", fontFamily: "Outfit, sans-serif", lineHeight: 1.3 }}>
                {publicaciones.length > 0 ? `${publicaciones.length} publicaciones` : "Publicaciones"} cerca de ti
              </p>
              <p style={{ margin: "0 0 12px", fontSize: 13, color: "#5b6080", fontFamily: "Outfit, sans-serif" }}>En un radio de 2 km</p>
            </div>
            <div style={{ background: "#7890D0", color: "white", borderRadius: 50, padding: "10px 18px", display: "inline-block", fontSize: 14, fontWeight: 700, fontFamily: "Outfit, sans-serif", width: "fit-content" }}>Ver Mapa →</div>
          </div>
          <div style={{ width: 110, borderRadius: 16, background: "#d0d4e8", overflow: "hidden", flexShrink: 0, minHeight: 130, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="#9aa0bc"/></svg>
          </div>
        </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: p.tipo === "compartir" ? "#f0f1f9" : "#fff0f2", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                      <path d="M4 9h13M4 9l3-3M4 9l3 3M20 15H7M20 15l-3-3M20 15l-3 3" stroke={p.tipo === "compartir" ? "#7890D0" : "#EC6765"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                  <div>
                    <p style={{ margin: "0 0 2px", fontWeight: 600, fontSize: 14, color: "#1e2a4a", fontFamily: "Outfit, sans-serif" }}>{p.nombre_insumo}</p>
                    <p style={{ margin: 0, fontSize: 11, color: "#b0b8d0", fontFamily: "Outfit, sans-serif" }}>{p.tipo === "compartir" ? "Compartiendo" : "Solicitando"} · hace poco</p>
                  </div>
                </div>
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#EC6765" strokeWidth="1.5"/><line x1="12" y1="9" x2="12" y2="13" stroke="#EC6765" strokeWidth="1.5" strokeLinecap="round"/><line x1="12" y1="17" x2="12.01" y2="17" stroke="#EC6765" strokeWidth="2" strokeLinecap="round"/></svg>
